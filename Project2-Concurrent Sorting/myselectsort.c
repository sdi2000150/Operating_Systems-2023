...
// Main function - Program for sorting records with SelectSort
int main(int argc, char* argv[]) {
    ...
    if (argc != 5) {
        ...
        fprintf(stderr, "   Usage: ./myselectsort DataFile starting_pos ending_pos Coordinator_PID\n");
        exit(1);
    }
    ...
    if (DataFilefd == -1) { 
        perror("    Program \"myselectsort\" failed to open DataFile"); 
        exit(1);
    }
    ...
    if (bytes_read == -1) {
        perror("    Program \"myselectsort\" failed to read from DataFile");
        free(records);
        close(DataFilefd);
        exit(1);
    }
    if (close(DataFilefd) == -1) { 
        perror("    Program \"myselectsort\" failed to close DataFile");
        free(records);
        exit(1);
    } 

    //Sorting slice of records
    SelectSort(records, num_of_records);
    ...
    for (int i = 0; i < num_of_records; i++) {
        if (write(1, &records[i], sizeof(Record)) == -1) {
            perror("    Program \"myselectsort\" failed to write to file descriptor of pipe");
        }
    }
    free(records);

    //Counting the time passed for the myselectsort program to finish its operations,
    t2 = (double) times(&tb2);
    ...
    if (kill(Coordinator_PID, SIGUSR2) == -1) {
        perror("    Program \"myselectsort\" failed to send a SIGUSR2 signal to Coordinator");
    }
